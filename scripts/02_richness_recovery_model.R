...
## From raw data rich, focal is scaled data fitted to model m1, all provided in results/richness_model.Rdata
## rich is the mean richness at each reef, per year, with regime (recovering/shifted) and protection (fished/no-take)

## scale and center response covariates
focal<-scaler(rich, ID = c('diff','Year', 'Location', 'base', 'site.year'))
...
save(m1, rich, focal, file='results/richness_model.Rdata')
